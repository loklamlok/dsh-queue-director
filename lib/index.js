...
const DIRECTIONS = ['up', 'down', 'top', 'bottom', 'to'];

/**
 * 把排队消息移到新位置。两条 splice 与内置 updateQueue 走同一条
 * `agent/inbox/spliced` 持久化通路，客户端快照会自动刷新。
 * direction 为 'to' 时按 index 移动（index 为移动后的最终位置，0-based）。
 * @returns {accepted: true} 或 {error: {code, message}}
 */
function reorderInbox(agent, itemId, direction, index) {
	const targets = [
		['next-turn', agent.inbox.nextTurn],
		['next-step', agent.inbox.nextStep]
	];
	for (const [target, list] of targets) {
		const idx = list.findIndex((message) => message.id === itemId);
		if (idx === -1) continue;
		const length = list.length;
		let to = idx;
		if (direction === 'up') to = idx - 1;
		else if (direction === 'down') to = idx + 1;
		else if (direction === 'top') to = 0;
		else if (direction === 'bottom') to = length - 1;
		else if (direction === 'to') {
			if (!Number.isInteger(index) || index < 0 || index >= length) {
				return { error: { code: 'bad-request', message: 'index out of range' } };
			}
			to = index;
		}
		if (to < 0 || to >= length || to === idx) return { accepted: true };
		const [moved] = agent.inbox.splice(target, idx, 1, []);
...
function buildApi(ctx) {
	...
	const agentOf = (sessionId) => {
		if (typeof sessionId !== 'string') return { error: { code: 'bad-request', message: 'sessionId is required' } };
		const agents = agentsOf();
		if (agents === void 0) return { error: { code: 'service-unavailable', message: 'agents service unavailable' } };
	...
	return {
		reorder(payload = {}) {
			const { sessionId, itemId, direction, index } = payload;
			...
			const found = agentOf(sessionId);
			if (found.error) return found;
			return reorderInbox(found.agent, itemId, direction, index);
		},
		/** 清空全部待处理消息（next-turn + next-step），不影响当前正在运行的 turn。 */
		clear(payload = {}) {
			...
			for (const message of [...agent.inbox.nextTurn]) {
				if (agent.inbox.remove(message.id)) removed += 1;
			}
